...
final class Stag_Customizer {
	...
	public function __construct() {
		...
		add_action( 'customize_controls_enqueue_scripts', array( $this, 'customizer_visibility_js' ) );
	...
	public function customizer_preview_js() {
		wp_enqueue_script( 'stag_customize_preview', get_template_directory_uri() . '/assets/js/customizer.js', array( 'customize-preview' ), '1.0', true );
	}

	/**
	 * Binds JS handlers for visibility settings in Theme Customizer
	 */
	public function customizer_visibility_js() {
		wp_enqueue_script( 'stag_customizer_visibility', get_template_directory_uri() . '/assets/js/customizer-visibility.js', array(), '1.0', true );
	}
 ...
 * @since 1.0
 *
 * @param string $section Name of the section under which option lives.
 * @param string $key The key of the theme mod to check. Prefixed with 'stag_'.
 * @param bool   $_default Where to retrieve the default value.
