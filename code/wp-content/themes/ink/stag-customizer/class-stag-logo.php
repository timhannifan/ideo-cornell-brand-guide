...
class Stag_Logo {
	...
	public function customize( $wp_customize ) {
		...
		$wp_customize->add_control(
			new WP_Customize_Image_Control(
				...
				'regular-logo-dark',
				array(
					'label'    => __( 'Regular Dark Logo', 'stag' ),
					'section'  => $section,
					'settings' => 'regular-logo-dark',
					'priority' => 10,
					'context'  => 'stag-custom-logo-regular-dark',
				)
			)
		);
		...
		$wp_customize->add_control(
			new WP_Customize_Image_Control(
				...
				'retina-logo-dark',
				array(
					'label'    => __( 'Retina Dark Logo', 'stag' ),
					'section'  => $section,
					'settings' => 'retina-logo-dark',
					'priority' => 20,
					'context'  => 'stag-custom-logo-retina-dark',
				)
			)
		);
	}
	 ...
	 *
	 * @param  tring $attachment_url The URL of the attached image.
	 ...
	 * @param bool   $force Force the cache to reset.
	 ...
	 */
	function has_logo() {
		return ( $this->has_logo_by_type( 'regular-logo' ) || $this->has_logo_by_type( 'retina-logo' ) || $this->has_logo_by_type( 'regular-logo-dark' ) || $this->has_logo_by_type( 'retina-logo-dark' ) );
	}
	...
	function print_logo_css() {
		...
		// CSS for displaying both dark mode logos.
		if ( $this->has_logo_by_type( 'regular-logo-dark' ) && $this->has_logo_by_type( 'retina-logo-dark' ) ) : ?>
			<?php $final_dimensions = $this->adjust_dimensions( $info['regular-logo']['width'], $info['regular-logo']['height'], $size, false ); ?>

			<style type="text/css" media="all">
				.header-normal .custom-logo {
					background: url("<?php echo addcslashes( esc_url_raw( $info['regular-logo-dark']['image'] ), '"' ); ?>") no-repeat;
					width: <?php echo absint( $final_dimensions['width'] ); ?>px;
					height: <?php echo absint( $final_dimensions['height'] ); ?>px;
					background-size: contain;
				}
				...
				(min-resolution: 1.3dppx) {
					.custom-logo {
						background: url("<?php echo addcslashes( esc_url_raw( $info['retina-logo-dark']['image'] ), '"' ); ?>") no-repeat;
						background-size: <?php echo absint( $final_dimensions['width'] ); ?>px <?php echo absint( $final_dimensions['height'] ); ?>px;
					}
				}
			</style>

		<?php elseif ( $this->has_logo_by_type( 'regular-logo-dark' ) ) : // CSS when ONLY the regular dark logo is available. ?>
			<?php $final_dimensions = $this->adjust_dimensions( $info['regular-logo-dark']['width'], $info['regular-logo-dark']['height'], $size ); ?>
			<style type="text/css" media="all">
				.header-normal .custom-logo {
					background: url("<?php echo addcslashes( esc_url_raw( $info['regular-logo-dark']['image'] ), '"' ); ?>") no-repeat;
					width: <?php echo absint( $final_dimensions['width'] ); ?>px;
		...
		<?php elseif ( $this->has_logo_by_type( 'retina-logo-dark' ) ) : // CSS when ONLY the retina dark logo is available. ?>
			<?php $final_dimensions = $this->adjust_dimensions( $info['retina-logo-dark']['width'], $info['retina-logo-dark']['height'], $size, true ); ?>
			<style type="text/css" media="all">
				.header-normal .custom-logo {
					background: url("<?php echo addcslashes( esc_url_raw( $info['retina-logo-dark']['image'] ), '"' ); ?>") no-repeat;
					width: <?php echo absint( $final_dimensions['width'] ); ?>px;
